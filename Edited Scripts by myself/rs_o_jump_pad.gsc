...
#define DEF_JUMP_PAD_Z_OFFSET 200
...
#precache( "fx", "custom/fx_spyro_sparks");
...
Desc : The first two functions got edited by myself where the original script initialized the jump pads on game start whereas in my map, they get initialized in a specific way by the growing soulboxes.
The init function gets called from the growibg_soulbox.gsc file and is given a parameter pTargetN for "Pad targetname" to initialize a set of Jump pads whenever conditions are met.
/////////////////////////////////////////
*/

function __init__(pTargetN)
{
	level jump_pad_init(pTargetN);
}

function jump_pad_init(pTargetN)
{
	jump_pad_triggers = GetEntArray( pTargetN, "targetname" );
...
function jump_pad_fx()
{
	...
	while( isdefined( self ) )
	{
		...
		playFxOnTag("custom/fx_spyro_sparks",fx,"tag_origin");
