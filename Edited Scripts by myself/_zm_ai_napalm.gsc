...
Desc : The following function handles the dancing napalm zombies in my map where is handled the trigger activation to make it dance and starts the music
/////////////////////////////////////////
*/

function make_zombie_dance()
{
	trig = GetEnt("trig_spawn_dancing_zombies", "targetname");
    ...
    platform = GetEnt("dancing_zombie_platform", "targetname");
	zombie = GetEnt("dancing_napalm_zombie", "targetname");
	zombie EnableLinkTo();
	zombie LinkTo(platform);
	platform MoveTo((platform.origin + (0, 0, 133)), 3, 2, 1);
	exploder::exploder("dancing_zombie_light");
	wait 3;
	zombie Unlink();
	zombie useanimtree(#animtree);
	zombie AnimScripted( "dancing_shoes1", zombie.origin , zombie.angles, %dancing_shoes1);
	zombie PlaySoundWithNotify("dancing_shoes_intro", "endmus");
	zombie waittill("endmus");
	zombie PlayLoopSound("dancing_shoes_loop");
	//ZMOVE : 133
}